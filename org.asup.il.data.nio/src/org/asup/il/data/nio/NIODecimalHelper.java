...
public class NIODecimalHelper {
	 ...
	 */
	private final static BigInteger TEN_TO_THE_SEVENTEENTH = BigInteger
	...
	public static final BigInteger[] MAX_DIGIT_ORIENTED_BI_VALUES = {
			BigInteger.ZERO, BigInteger.valueOf(9), BigInteger.valueOf(99),
			BigInteger.valueOf(999), BigInteger.valueOf(9999),
			BigInteger.valueOf(99999), BigInteger.valueOf(999999),
			BigInteger.valueOf(9999999), BigInteger.valueOf(99999999),
			BigInteger.valueOf(999999999), BigInteger.valueOf(9999999999L),
			BigInteger.valueOf(99999999999L),
			BigInteger.valueOf(999999999999L),
			BigInteger.valueOf(9999999999999L),
	...
	public static final BigInteger[] MIN_DIGIT_ORIENTED_BI_VALUES = {
			BigInteger.ZERO, MAX_DIGIT_ORIENTED_BI_VALUES[1].negate(),
	...
	public static BigDecimal asDecimal(BigDecimal value, int precision,
			int scale) {
		if (value == null)
			return null;

		return asDecimal(value, getMaxValue(precision, scale),
				getMinValue(precision, scale), precision, scale, false);
	}
	...
	public static BigDecimal asDecimal(BigDecimal value, BigDecimal max,
			BigDecimal min, int precision, int scale, boolean ignoreOverflow) {
		...
		if (ignoreOverflow)
			...
			if ((result.compareTo(max) <= 0 && result.compareTo(min) >= 0))
				...
				return handleNumericOverflow(value, precision, scale,
						ignoreOverflow);
		}
	}
	 ...
	 * @throws AnyException
	 */
	private static BigDecimal handleNumericOverflow(BigDecimal value,
			int precision, int scale, boolean ignoreOverflow) {
		BigDecimal result = value;
		if (ignoreOverflow) {
			...
			result = value.subtract(value.divide(divisor, 0,
					BigDecimal.ROUND_DOWN).multiply(divisor));

		} else {
			// The program wants an exception to be thrown.
			throw new FrameworkCoreRuntimeException("Numeric overflow");

			// String message =
			// org.eclipse.edt.javart.util.JavartUtil.errorMessage(
			// program,
			// Message.EXPRESSION_OVERFLOW,
			// new Object[]{value.toString() + " as decimal(" + precision + ", "
			// + scale + ")"});
	 ...
	 */
	public static BigInteger decimalToBigInteger(byte[] buffer, int offset,
			int length) {
		...
		if (top > 9) {
			throw new FrameworkCoreRuntimeException("Invalid conversion");
		}
	 ...
	 */
	public static void toDecimal(int value, byte[] buffer, int offset,
			int length, int itemLength, boolean packed) {
