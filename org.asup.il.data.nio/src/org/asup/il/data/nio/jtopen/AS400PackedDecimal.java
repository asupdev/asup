package org.asup.il.data.nio.jtopen;

import java.math.BigDecimal;
import java.math.BigInteger;

/**
 * Provides a converter between a BigDecimal object and a packed decimal format
...
public class AS400PackedDecimal implements AS400DataType {
	 ...
	 * 
	 * @param numDigits
	 *            The number of digits in the packed decimal number. It must be
	 ...
	 *            It must be greater than or equal to zero and less than or
	 *            equal to <i>numDigits</i>.
	 **/
	...
	/**
	 * Creates a new AS400PackedDecimal object that is identical to the current
	 * instance.
	 * 
	...
	/**
	 * Returns {@link com.ibm.as400.access.AS400DataType#TYPE_PACKED
	 * TYPE_PACKED}.
	 * 
	 ...
	 * {@link java.math.BigDecimal BigDecimal} object will be returned on a call
	 * to {@link #toObject toObject()}.
	 * 
	 * @return true if a Double will be returned, false if a BigDecimal will be
	 ...
	 * 
	 * @param javaValue
	 *            The object corresponding to the data type. It must be an
	 *            instance of BigDecimal and the BigDecimal must have a less
	 *            than or equal to number of digits and a less than or equal to
	...
	/**
	 * Converts the specified Java object into IBM i format in the specified
	 * byte array.
	 * 
	 * @param javaValue
	 *            The object corresponding to the data type. It must be an
	 ...
	 *            number of decimal places.
	 * @param as400Value
	 *            The array to receive the data type in IBM i format. There must
	 ...
	 * 
	 * @param javaValue
	 *            An object corresponding to the data type. It must be an
	 *            instance of BigDecimal and the BigDecimal must have a less
	 *            than or equal to number of digits and a less than or equal to
	 *            number of decimal places.
	 * @param as400Value
	 ...
	 *            be enough space to hold the IBM i value.
	 * @param offset
	 *            The offset into the byte array for the start of the IBM i
	...
	public int toBytes(Object javaValue, byte[] as400Value, int offset) {
		...
		try {
			inValue = (BigDecimal) javaValue; // Let this line throw
												// ClassCastException
		...
		// get just the digits from BigDecimal, "normalize" away sign, decimal
		// place etc.
		...
		// write correct number of leading zero's, allow ArrayIndexException to
		// be thrown below
		...
		if (leadingZeros > 0) {
			...
			System.err.println("The calculated number of leading zeros is negative. "+ leadingZeros);
		}
	 ...
	 *
	 * @param doubleValue
	 *            The value to be converted to IBM i format. If the decimal part
	 *            of this value needs to be truncated, it will be rounded
	 *            towards zero. If the integral part of this value needs to be
	 *            truncated, an exception will be thrown.
	 * @return The IBM i representation of the data type.
	 **/
	 ...
	 * Java double value, then it is rounded towards zero. If the integral part
	 * of the value needs to be truncated to be represented by a Java double
	 * value, then it converted to either Double.POSITIVE_INFINITY or
	 * Double.NEGATIVE_INFINITY.
	 * 
	 * @param as400Value
	 *            The array containing the data type in IBM i format. The entire
	 *            data type must be represented.
	...
	public double toDouble(byte[] as400Value, int offset) {
		...
		case 0x000F:
			...
			break;
		default:
			throwNumberFormatException(LOW_NIBBLE, rightMostOffset, as400Value[rightMostOffset] & 0x00FF, as400Value);
		}
	 ...
	 * @param as400Value
	 *            The array containing the data type in IBM i format. The entire
	 *            data type must be represented and the data type must have
	 *            valid packed decimal format.
	 * @param offset
	 *            The offset into the byte array for the start of the IBM i
	 *            value. It must be greater than or equal to zero.
	 * @return The BigDecimal object corresponding to the data type.
	...
	public Object toObject(byte[] as400Value, int offset) {
		if (useDouble_)
			return new Double(toDouble(as400Value, offset));
		...
		default: // others invalid
			throwNumberFormatException(LOW_NIBBLE, offset + inputSize - 1, as400Value[offset + inputSize - 1] & 0xFF, as400Value);
		}
		...
		while (outputPosition < outputData.length - 1) {
			...
			if (nibble > 0x09)
				throwNumberFormatException(HIGH_NIBBLE, offset, as400Value[offset] & 0xFF, as400Value);
			outputData[outputPosition++] = (char) (nibble | 0x0030);

			nibble = (as400Value[offset++] & 0x0F);
			if (nibble > 0x09)
				throwNumberFormatException(LOW_NIBBLE, offset - 1, as400Value[offset - 1] & 0xFF, as400Value);
			outputData[outputPosition++] = (char) (nibble | 0x0030);
		}
	...
	static final void throwNumberFormatException(boolean highNibble, int byteOffset, int byteValue, byte[] fieldBytes) throws NumberFormatException {
		String text;
		if (highNibble) {
			text = "EXC_HIGH_NIBBLE_NOT_VALID";
		} else {
			text = "EXC_LOW_NIBBLE_NOT_VALID";
		}
		System.err.println("Byte sequence is not valid for a field of type 'packed decimal': "+ fieldBytes);
		throw new NumberFormatException(text);
	}

	public static final String byteToString(int byteVal) {
