...
/**
 * The AS400ZonedDecimal class provides a converter between a BigDecimal object
 * and a zoned decimal format floating point number.
 **/
public class AS400ZonedDecimal implements AS400DataType {
	 ...
	 * 
	 * @param numDigits
	 *            The number of digits in the zoned decimal number. It must be
	...
	/**
	 * Creates a new AS400ZonedDecimal object that is identical to the current
	 * instance.
	 * 
	...
	public double toDouble(byte[] as400Value, int offset) {
		...
		// Compute the value.
		/*
		 * This old code had a bug in that it can produce inexact answers. For
		 * example 10.10105 is turned into -10.101049999999999
		 * 
		 * double doubleValue = 0; double multiplier = Math.pow(10, digits -
		 * scale - 1); int rightMostOffset = offset + digits - 1; for(int i =
		 * offset; i <= rightMostOffset; ++i) { doubleValue +=
		 * ((byte)(as400Value[i] & 0x000F)) * multiplier; multiplier /= 10; }
		 */
		...
		case 0x00F0:
			...
			break;
		default:
			throwNumberFormatException(HIGH_NIBBLE, rightMostOffset, as400Value[rightMostOffset] & 0x00FF, as400Value);
		}
	...
	public Object toObject(byte[] as400Value, int offset) {
		...
		default: // others invalid
			throwNumberFormatException(HIGH_NIBBLE, offset + size - 1, as400Value[offset + size - 1] & 0xFF, as400Value);
		}
		...
		while (outputPosition < outputData.length) {
			...
			if (nibble > 0x0009)
				throwNumberFormatException(LOW_NIBBLE, offset - 1, as400Value[offset - 1] & 0x00FF, as400Value);
	...
	static final void throwNumberFormatException(boolean highNibble, int byteOffset, int byteValue, byte[] fieldBytes) throws NumberFormatException {
		...
		System.err.println("Byte sequence is not valid for a field of type 'zoned decimal': "+ fieldBytes);
